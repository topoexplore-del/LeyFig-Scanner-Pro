"""
LEYFIG SCANNER PRO — Multi-Layer Alert System v18.0
...
# LEYFIG — 5ª capa: madurez máxima del movimiento para alertar (primer tercio).
...
def validate_all(r):
    ...
    result = {'ticker': r.get('ticker','?'), 'name': r.get('name','?'), 'price': r.get('close',0),
              ...
              'setup': r.get('setup'), 'setup_icon': r.get('setup_icon'), 'setup_nombre': r.get('setup_nombre'),
              'setup_desc': r.get('setup_desc'), 'setup_trigger': r.get('setup_trigger'),
              'stage': r.get('stage'), 'stage_rank': r.get('stage_rank'), 'etapa': r.get('etapa'), 'etapa_icon': r.get('etapa_icon'),
              'leg_hi': r.get('leg_hi'), 'leg_lo': r.get('leg_lo'), 'leg_pct': r.get('leg_pct'),
              'run_atr': r.get('run_atr'), 'upside_left': r.get('upside_left'),
    ...
    atr = r.get('atr')
    if atr and atr > 0:
        # LEYFIG — entrada por RETROCESO DE 1/3 del impulso vigente.
        # En vez de comprar al precio de mercado (que en un tramo ya corrido
        # deja el stop lejísimos), se espera a que el precio devuelva un tercio
        # del recorrido. Eso mejora el riesgo/beneficio y evita perseguir.
        # Salvaguarda: nunca se pide un retroceso mayor a 2 ATR (no llenaría).
        stp = r.get('setup')
        trig = r.get('setup_trigger')
        piso, techo = close - 2.0*atr, close - 0.10*atr
        if stp in ('PULLBACK', 'GIRO'):
            # El retroceso YA ocurrió: pedir más descuento sería no entrar nunca.
            entry, modo = close - 0.2*atr, 'al giro (ya corregido)'
        elif stp == 'RUPTURA' and trig:
            # Retest del techo de la base roto: la entrada natural del breakout.
            entry, modo = max(min(float(trig), techo), piso), 'retest de la ruptura'
        elif stp == 'PRERUPTURA':
            # Se compra la compresión, antes de que el movimiento arranque.
            entry, modo = close - 0.2*atr, 'dentro de la base'
        else:
            leg_hi, leg_lo = r.get('leg_hi'), r.get('leg_lo')
            entry, modo = close - 0.3*atr, 'pullback ATR'
            if leg_hi and leg_lo and leg_hi > leg_lo:
                cand = max(min(leg_hi - (leg_hi - leg_lo)/3.0, techo), piso)
                if cand < entry:
                    entry, modo = cand, 'retroceso 1/3'
        entry = round(entry, 2)
        result['entry_mode'] = modo
        result['pullback_pct'] = round((entry/close - 1)*100, 2) if close else None
...
def build_telegram_msg(confirmed, near_miss, timestamp, regime_info=None):
    ...
    for a in confirmed:
        ...
        if a.get('setup'):
            msg += f"{a.get('setup_icon','')} *{a.get('setup_nombre','')}*\n_{a.get('setup_desc','')}_\n"
        if a.get('stage') is not None:
            msg += f"{a.get('etapa_icon','')} Etapa del movimiento: *{a.get('etapa','')}* ({a['stage']:.0f}% recorrido)"
            if a.get('upside_left') is not None:
                msg += f" · recorrido pendiente ≈ {a['upside_left']:+.0f}%"
            msg += "\n"
        if a.get('entry_mode'):
            msg += f"⏳ Entrada: {a['entry_mode']} ({a.get('pullback_pct',0):.1f}% bajo el precio)\n"
        st = a.get('status')
        if st:
...
def build_email_html(confirmed, near_miss, timestamp):
    ...
    <div style="background:#12131a;border:1px solid #2a2b36;border-radius:8px;padding:20px;margin-bottom:16px;text-align:center">
        <h1 style="color:#00e676;margin:0;font-size:22px;letter-spacing:2px">🎯 LEYFIG SCANNER PRO</h1>
    ...
    html += f"""<div style="text-align:center;padding:16px;color:#5a5c66;font-size:10px;border-top:1px solid #2a2b36">
        <p style="margin:0">LEYFIG SCANNER PRO v18.0 · Solo LONG · 4-Layer Validation</p>
...
def main():
    ...
    print(f"LEYFIG SCANNER PRO — Alert Check · {timestamp}\n{'='*55}")
    ...
    print(f"  🧭 LeyFig: solo arquetipos de entrada temprana (pre-ruptura · ruptura ≤5d · pullback · giro)")
    if regime == 'BEAR':
    ...
    for gn, stocks in snapshot.get('groups', {}).items():
        for r in stocks:
            ...
            # LEYFIG — 5ª capa: descartar movimientos ya desarrollados.
            # 5ª CAPA — MOMENTO DEL MOVIMIENTO (el corazón de LeyFig)
            # (a) Debe existir un arquetipo de entrada temprana. Sin él no hay
            #     señal, por alto que sea el score: comprar fuerza ya desplegada
            #     es exactamente lo que se quiere evitar.
            # (b) PULLBACK/GIRO/PRE-RUPTURA son tempranos por construcción.
            #     La RUPTURA además debe estar en el primer tercio de madurez.
            stp, stg, srank = r.get('setup'), r.get('stage'), r.get('stage_rank')
            motivo = None
            if not stp:
                motivo = "Sin setup temprano (movimiento ya desarrollado)"
            elif stp == 'RUPTURA' and srank is not None and srank > EARLY_STAGE_MAX:
                motivo = f"Ruptura ya avanzada (madurez {stg:.0f}%)" if stg is not None else "Ruptura avanzada"
            elif stg is not None and stg > 85:
                motivo = f"Tramo agotado (madurez {stg:.0f}%)"
            if res['all_passed'] and motivo:
                res['all_passed'] = False
                res['failed'].append(motivo)
                res['layers_passed'] = 3   # visible en watchlist con su motivo
    ...
    with open(alerts_path, 'w') as f:
        json.dump(json_safe({'checked_at': timestamp, 'regime': regime_info, 'confirmed': len(confirmed),
                   'signals': [{'ticker':a['ticker'],'composite':a['composite'],'prob':a['prob'],
                               ...
                               'setup':a.get('setup'),'setup_icon':a.get('setup_icon'),'setup_nombre':a.get('setup_nombre'),'setup_desc':a.get('setup_desc'),
                               'stage':a.get('stage'),'stage_rank':a.get('stage_rank'),'etapa':a.get('etapa'),'etapa_icon':a.get('etapa_icon'),
                               'upside_left':a.get('upside_left'),'entry_mode':a.get('entry_mode'),'pullback_pct':a.get('pullback_pct'),
