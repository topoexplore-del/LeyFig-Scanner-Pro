# 🎯 LeyFig Scanner Pro v19 — Edición Profesional
...
## 🎯 Novedad LeyFig — Madurez del Movimiento

LeyFig conserva toda la metodología de AndFig y añade la capa que faltaba: **medir en qué punto del recorrido está cada movimiento** para entrar en su primer tercio, no en el último.

- **Índice de Madurez (0-100)** por acción: recorrido en ATRs, extensión sobre la SMA20, velas transcurridas, avance hacia el objetivo del measured move y RSI. El impulso se mide desde donde el precio **abandona la base**, no desde el mínimo — así una ruptura reciente tras meses de lateral cuenta como TEMPRANA.
- **Etiqueta relativa al universo**: 🟢 TEMPRANA (percentil ≤ 33), 🟡 MEDIA, 🔴 TARDÍA. El filtro se autocalibra al mercado de cada día.
- **5ª capa de validación**: solo se alertan movimientos del primer tercio; los descartados aparecen en la watchlist con su motivo.
- **Detector de arquetipos de entrada**: la señal solo nace si el gráfico está en uno de los cuatro puntos de arranque — 🎯 pre-ruptura, 🚀 ruptura ≤5 velas, ↩️ pullback en tendencia o 🔄 giro desde sobreventa. Sin arquetipo no hay alerta, por alto que sea el score.
- **Entrada adaptada al arquetipo**: retest en rupturas, al giro en pullbacks, dentro de la base en pre-rupturas.
- **Screener ordenado por madurez ascendente** por defecto, con filtros de etapa y columna dedicada.

Detalle completo en `MANUAL_MADUREZ.md`.
